...
import 'package:user/beanmodel/ratting/rattingbean.dart';
import 'package:user/beanmodel/storefinder/storefinderbean.dart';
import 'package:user/beanmodel/whatsnew/whatsnew.dart';
import 'package:user/beanmodel/wishlist/addorremovewish.dart';
...
class ProductSearchInfo extends StatefulWidget {
  @override
  _ProductSearchInfoState createState() => _ProductSearchInfoState();
}

class _ProductSearchInfoState extends State<ProductSearchInfo> {
  ...
  void getNotiProduct(dynamic productid) async {
    if(locEmitterP.state.lat!=0.0 && locEmitterP.state.lng!=0.0){
      print('in ps 1');
      storedetails = locEmitterP.state.storeFinderData;
      hitTheStore(productid);
    }else{
      print('in ps 2');
      locEmitterP.hitLocEmitterInitial();
      locEmitterP.stream.first.then((value) {
        if(value!=null){
          print('in ps 3');
          storedetails = value.storeFinderData;
          hitTheStore(productid);
        }else{
          print('in ps 4');
          isSearching = false;
          shownMessage = 'No product found';
          setState(() {});
        }
      }).catchError((e){
        print('in ps 5');
  ...
  void hitTheStore(dynamic productid) async{
    SharedPreferences prefs = await SharedPreferences.getInstance();
    http.post(notiProductUri, body: {
      // 'lat': '23.060543879153062',
      'lat': '${locEmitterP.state.lat}',
      // 'lng': '72.6823303553762',
      'lng': '${locEmitterP.state.lng}',
      ...
      if (value.statusCode == 200) {
        ...
        if ('${data1.status}' == '1') {
          ...
          if (productDetails.varients != null && productDetails.varients.isNotEmpty) {
            varaintList.clear();
            varaintList = List.from(productDetails.varients);
            print('${productDetails.varientId}');
            selectedIndex = varaintList.indexOf(ProductVarient(varientId: productDetails.varientId));
          ...
          if (cartItemd != null && cartItemd.isNotEmpty) {
          ...
          isWishList = false;
          if (productDetails.tags != null && productDetails.tags.isNotEmpty) {
            tagsList.clear();
  ...
  Widget build(BuildContext context) {
    ...
    if (!enterFirst) {
      ...
      var productId = receivedData['prodid'];
      getNotiProduct(productId);
    }
    ...
    return SafeArea(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text('${(productDetails!=null)?productDetails.productName:'Searching'}',
          style: TextStyle(
        ...
        body: isSearching
            ...
            :(productDetails!=null)?BlocBuilder<CartListProvider, List<CartItemData>>(
