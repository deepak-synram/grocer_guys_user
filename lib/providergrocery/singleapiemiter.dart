...
class SingleApiEmitter extends Cubit<SingleApiEmitterBeanModel> {
  SingleApiEmitter()
      : super(SingleApiEmitterBeanModel(
            isSearching: false, dataModel: SingleApiHomePage(status: '0')));

  void hitSingleApiEmitter(
      String storeId, RefreshController refreshController) async {
    getSingleAPi(storeId, refreshController);
  }

  void getSingleAPi(String storeId, RefreshController refreshController) async {
    emit(SingleApiEmitterBeanModel(
        isSearching: true, dataModel: SingleApiHomePage(status: '0')));
    SharedPreferences prefs = await SharedPreferences.getInstance();
    var http = Client();
    http.post(oneApiUri, body: {
      'store_id': '$storeId'
    }, headers: {
      'Authorization': 'Bearer ${prefs.getString('accesstoken')}'
    }).then((value) {
      print('Something happened !!!!!');
      print("The Value of the body is : " + value.body);
      print("The Status code is :  ${value.statusCode}");
      if (value.statusCode == 200) {
        SingleApiHomePage data1 =
            SingleApiHomePage.fromJson(jsonDecode(value.body));
        print('Status heree !! ${data1.status}');
        if ('${data1.status}' == '1') {
          ...
          emit(SingleApiEmitterBeanModel(
              isSearching: false,
              dataModel:
                  SingleApiHomePage(status: '0', message: data1.message)));
        }
      } else {
        emit(SingleApiEmitterBeanModel(
            isSearching: false, dataModel: SingleApiHomePage(status: '0')));
